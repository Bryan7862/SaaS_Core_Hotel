...
export class CulqiWebhookController {
    ...
    @Post()
    async handleWebhook(@Body() payload: any, @Headers('culqi-signature') signature: string) {
        ...
        if (payload?.type !== 'charge.creation.succeeded' && payload?.type !== 'charge.creation.failed') {
        ...
        if (existingTx) {
            this.logger.log(`Idempotency Check: Charge ${chargeId} already processed.`);
        ...
        // A. Handle Failures
        if (payload.type === 'charge.creation.failed' || charge.outcome.type !== 'venta_exitosa') {
            this.logger.warn(`Charge ${chargeId} FAILED. Outcome: ${charge.outcome.type}`);
        ...
        if (planCode === PlanCode.BASIC) expectedAmount = 50.00;
        else if (planCode === PlanCode.PRO) expectedAmount = 100.00;
        else if (planCode === PlanCode.MAX) expectedAmount = 200.00;

        // Allow small margin? (e.g. 50 vs 50.00). Strict for now.
        if (expectedAmount > 0 && amount < expectedAmount) {
            this.logger.error(`Price Manipulation Attempt! Paid ${amount} for ${planCode} (Expected ${expectedAmount})`);
            await this.transactionsService.createPaymentLog({
